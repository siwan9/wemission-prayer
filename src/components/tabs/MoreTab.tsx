...
const MoreTab: React.FC<MoreTabProps> = ({linksData}) => {
  const menus = [
    { name: '2026 캠프 찬양 리스트', icon: 'fa-music', url: linksData.songListUrl, color: 'bg-indigo-600', subtitle: '캠프를 준비하며 함께 찬양해요' },
    { name: '2026 캠프 찬양 악보', icon: 'fa-file-lines', url: linksData.songSheetUrl, color: 'bg-teal-600', subtitle: '예배를 위한 악보 다운로드' },
    { name: '2026 겨울 위미션 캠프소개', icon: 'fa-campground', url: linksData.campIntroUrl, color: 'bg-red-500', subtitle: '캠프 일정 및 상세 안내' },
    ...
    <div className="grid gap-4 md:grid-cols-2 max-w-4xl mx-auto animate-fadeIn">
      {menus.map((menu, i) => (
        <a 
          ...
          className="flex flex-col bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl transition-all group"
        >
          <div className="flex items-center gap-4 p-5">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-white font-bold block truncate">{menu.name}</span>
              <span className="text-[10px] text-gray-500">{menu.subtitle}</span>
            </div>
